...
export default {
  ...
  created() {
     ...
     * Fetch all programs.
     */
    const fetchPrograms = this.$store.dispatch("fetchPrograms").then(res => {
      res.forEach(program => {
     ...
     */
    const getOption = this.$store
      .dispatch("getOption", this.$route.params.id)
      .then(res => {
     ...
     */
    Promise.all([fetchPrograms, fetchQuestions, getOption]).then(() => {
